...
const NotificationsItem = ({item, onMarkAsRead}) => {
  ...
  return (
   <TouchableOpacity style={[styles.itemContainer, !item.isRead && styles.itemUnreadBackground]} onPress={() => onMarkAsRead(item.id)}>
    <View style={styles.notificationsContent}>
        { !item.isRead && <Ionicons name="ellipse" size={10} color="blue" style={styles.unreadIndicator} /> }
        ...
        <Text style={styles.dateText}>{new Date(safeDateString).toLocaleTimeString('pt-BR', {hour: '2-digit', minute: '2-digit'})}</Text>
    </View>
    </TouchableOpacity>
    )
};
